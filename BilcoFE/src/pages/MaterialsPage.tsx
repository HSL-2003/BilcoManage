    ...
    <MainLayout>
      <div className="admin-root">
        <section className="admin-header">
          <div>
            <h2 className="admin-title">Quản lý tồn kho (TonKho)</h2>
            <p className="admin-subtitle">
              Theo dõi số lượng tồn, khả dụng và vị trí lưu trữ của vật tư.
            </p>
          </div>
          <button className="btn-admin-primary" onClick={handleAddNew}>+ Thêm tồn kho</button>
        </section>

        <section className="admin-card animate-up">
          <div className="admin-card-header">
            <h3 className="admin-card-title">Danh sách tồn kho</h3>
              ...
              <tbody>
                {items.length === 0 ? (
                    ...
                    items.map(item => (
                        ...
                        <td>#{item.maTonKho}</td>
                        <td><span style={{fontWeight: 600}}>VT-{item.maVT}</span></td>
                        <td>K-{item.maKho}</td>
                        <td style={{fontWeight: 'bold', color: '#1a73e8'}}>{item.soLuongTon}</td>
                        <td>{item.soLuongKhaDung}</td>
                        <td style={{color: item.soLuongTon < item.soLuongToiThieu ? 'red' : 'inherit'}}>
                            {item.soLuongToiThieu}
                            {item.soLuongTon < item.soLuongToiThieu && <span className="badge badge-danger" style={{marginLeft: '8px'}}>Thấp</span>}
                        </td>
                        ...
                        <td>
                            <div style={{display: 'flex', gap: '8px'}}>
                                ...
                                <button className="btn-admin-danger" onClick={() => handleDelete(item.maTonKho)}>Xóa</button>
                            </div>
                        </td>
                        </tr>
                    ))
        ...
        {isModalOpen && (
             <div className="modal-overlay">
                <div className="modal-content" style={{width: '600px'}}>
                    <h3 style={{marginTop: 0}}>{editingId ? 'Cập nhật tồn kho' : 'Thêm tồn kho mới'}</h3>
